...
# CSS for background image and dark sidebar
background_image = """
<style>
...
[data-testid="stSidebar"] .css-1y4p8pa, .css-1y4p8pa {
  padding-top: 2rem;
}
.css-1y4p8pa * {
  color: white;
...
st.sidebar.image(image='jose-martin-ramirez-carrasco-45sjAjSjArQ-unsplash.jpg', use_column_width=True)

st.sidebar.title('Census Data Exploration')
st.sidebar.write('Use the controls below to select options and filter the data.')

selected_year = st.sidebar.slider('Select a year for Census data', 2011, 2021, 2018)
...
if not df.empty:
    ...
    st.subheader('Histogram of Median Family Income')
    ...
    st.subheader(f'Scatterplot of Median Family Income by Percent Unemployed in {selected_year}')
    plt.figure(figsize=(10, 6))
    ...
    # Sidebar selection for lmplot analysis
    st.sidebar.subheader(f'Please select an X and Y value for analysis in {selected_year}' )
    ...
    st.subheader(f'lmplot analysis by Income Quartile in {selected_year}')
